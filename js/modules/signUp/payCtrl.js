...
define(['angular', '../js/services/signUpService.js'], function (angular, signUpService) {
    ...
    app.controller("payCtrl", ['$scope', '$sce', '$timeout', function ($scope, $sce, $timeout) {
        ...
        $scope.weChatPayUrl = $sce.trustAsResourceUrl(appConfig.weChatRootUrl + "Pay/NativePayPage.aspx?orderIds=" + utility.getQueryParameter("orderIds"));
        ...
        $scope.confirmOrder = function () {
            signUpService.postFreeOrders($scope.orderIds).$promise.then(function (response) {
                if (response.Code == 0) {
                    appConfig.goToCompletePage($scope.orderIds);
                } else {
